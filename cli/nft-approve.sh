...
"$SPENDER" "$TOKEN" $(date -d "$DURATION" +%s%N) # approval for 2 minutes
        APPROVAL_CMD="$CLI tx wasm execute $COLLECTION '$MSG' --from $WALLET_MINTER --gas-prices $GAS_PRICES --gas $GAS --gas-adjustment $GAS_ADJUSTMENT -b $BROADCAST_MODE --yes --chain-id $CHAIN_ID --node $CHAIN_NODE"
    else

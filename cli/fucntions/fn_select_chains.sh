...
function fn_select_chains() {
  ...
  # Display the menu
  echo "Please choose a chain by entering its number:"
  for i in "${!all_options[@]}"; do
    echo "$i) ${all_options[$i]}"
  done
  ...
  while true; do
    if ! [[ "$choice" =~ ^[0-9]+$ ]] || [ "$choice" -lt 0 ] || [ "$choice" -ge ${#all_options[@]} ]; then
      echo "Invalid input. Please enter a number between 0 and $((${#all_options[@]} - 1))."
    else
      break
    fi
  done
  ...
  if [ "$choice" -eq 0 ]; then
    ...
    echo "You selected: ${all_options[$choice]}"

...
# Stargaze
CW721_CONTRACT=`"$ARK_CLI_DIR"/create-collection.sh --chain stars --name "$NAME" --symbol "$SYMBOL" --label "$LABEL" | tail -n 1 2>&1 | tee -a logs/collections.log`
REPLACE_PATTERN="s/GON_STARGAZE_CONTRACT_CW721=\".*\"/GON_STARGAZE_CONTRACT_CW721=\"$CW721_CONTRACT\"/"
sed -i "$REPLACE_PATTERN" ./nodes.env

# Juno
CW721_CONTRACT=`"$ARK_CLI_DIR"/create-collection.sh --chain juno --name "$NAME" --symbol "$SYMBOL" --label "$LABEL" | tail -n 1 2>&1 | tee -a logs/collections.log`
REPLACE_PATTERN="s/GON_JUNO_CONTRACT_CW721=\".*\"/GON_JUNO_CONTRACT_CW721=\"$CW721_CONTRACT\"/"
sed -i "$REPLACE_PATTERN" ./nodes.env

# Osmosis
CW721_CONTRACT=`"$ARK_CLI_DIR"/create-collection.sh --chain osmo --name "$NAME" --symbol "$SYMBOL" --label "$LABEL" | tail -n 1 2>&1 | tee -a logs/collections.log`
REPLACE_PATTERN="s/GON_OSMOSIS_CONTRACT_CW721=\".*\"/GON_OSMOSIS_CONTRACT_CW721=\"$CW721_CONTRACT\"/"
sed -i "$REPLACE_PATTERN" ./nodes.env

# IRISnet
"$ARK_CLI_DIR"/create-collection.sh --chain iris \
...
# Uptick
"$ARK_CLI_DIR"/create-collection.sh --chain upt \
    --collection "$COLLECTION_ID" \
    --uri "$URI" \
    --name "$NAME" \
    --symbol "$SYMBOL" \
...
# OmniFlix
"$ARK_CLI_DIR"/create-collection.sh --chain omni \
    --collection "$COLLECTION_ID" \
    --uri "$URI" \
    --name "$NAME" \
    --symbol "$SYMBOL" \
